...
class ColumnSchema(Schema):
    ...
    field_type = fields.String(required=True, validate=validate.OneOf(["text", "date", "stage"]))
...
class ColumnUpdateSchema(Schema):
    ...
    field_type = fields.String(validate=validate.OneOf(["text", "date", "stage"]))
...
def cell_dict(cell):
    if cell is None:
        return {"version": 0, "text_value": None, "date_value": None,
                "planned_date": None, "is_completed": False, "completed_date": None}
    result = {key: getattr(cell, key) for key in (
        "version", "text_value", "date_value", "planned_date", "is_completed", "completed_date"
    )}
...
def save_cell(plan, column, payload):
    ...
    allowed = {"text": {"text_value"}, "date": {"date_value"},
               "stage": {"planned_date", "is_completed", "completed_date"}}[column.field_type]
    ...
    if "is_completed" in values and values["is_completed"] is None:
