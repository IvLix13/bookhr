...
Bookuchet отправляет уведомления через отдельного пользователя Nextcloud и совместим
с Nextcloud 26. Создайте сервисного пользователя (например, `bookhr-bot`), войдите под
ним и выпустите пароль приложения в разделе «Настройки → Безопасность». Обычный пароль
пользователя в конфигурации не храните.

Добавьте в `.env.prod`:

```dotenv
NEXTCLOUD_BASE_URL=https://nextcloud.example.local
NEXTCLOUD_USERNAME=bookhr-bot
NEXTCLOUD_APP_PASSWORD=generated-app-password
NEXTCLOUD_VERIFY_SSL=false
```

Если Nextcloud установлен в подпапке, включите её в `NEXTCLOUD_BASE_URL`, например
`https://server/nextcloud`. После изменения перезапустите `bookuchet`; служба
`bookuchet-notifications.service` прочитает тот же EnvironmentFile при следующем запуске.

В интерфейсе Bookuchet администратор или HR ищет получателя по ФИО. Поиск выполняется
через autocomplete Nextcloud. После выбора сохраняется неизменяемый Nextcloud user ID,
а при отправке создаётся или повторно используется личный Talk-диалог сервисного
пользователя с получателем.

При переходе со старой схемы комнат миграция отключает существующие правила: токен
комнаты нельзя безопасно преобразовать в пользователя. После обновления откройте
«Настройки → Уведомления Nextcloud», выберите получателей и снова включите правила.

Для внутреннего Nextcloud с самоподписанным или недоверенным сертификатом проверка SSL отключается:

```dotenv
NEXTCLOUD_VERIFY_SSL=false
```

Настройка действует только на исходящие запросы Bookuchet к Nextcloud. Не используйте
`false` для подключения через недоверенные сети: без проверки сертификата соединение уязвимо
для перехвата. После установки доверенного сертификата проверку можно вернуть значением `true`.
