...
Для внутреннего Nextcloud с самоподписанным или недоверенным сертификатом проверка SSL отключена:

```dotenv
NEXTCLOUD_VERIFY_SSL=false
```

Настройка действует только на исходящие запросы Bookuchet к Nextcloud Talk. Не используйте
